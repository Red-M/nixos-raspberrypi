...
Manages Raspberry Pi firmware partition `/boot/firmware` (the path is configurable with `boot.loader.nixosRaspberryPi.firmwarePath`).
...
Supported boot methods (configurable with `boot.loader.nixosRaspberryPi.bootloader`):
...
Configuration options for the bootloader are in `boot.loader.nixosRaspberryPi` (defined in `modules/system/boot/loader/raspberrypi/default.nix`).
...
> to keep that in your configuration, set `boot.loader.nixosRaspberryPi.bootloader = "kernel"`.
...
This is basically [`boot.loader.nixosRaspberryPi` options](https://search.nixos.org/options?channel=unstable&show=boot.loader.nixosRaspberryPi), which are deprecated in nixpkgs, but updated and improved upon.
...
We found [`boot.loader.nixosRaspberryPi` options](https://search.nixos.org/options?channel=unstable&show=boot.loader.nixosRaspberryPi) to be much more idiomatic, easier to extend, and maintain.
