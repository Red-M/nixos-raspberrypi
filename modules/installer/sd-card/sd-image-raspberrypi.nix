...
{ config, pkgs, lib, modulesPath, ... }: {
  ...
  boot.loader.nixosRaspberryPi.bootloader = lib.mkIf
    (config.boot.loader.nixosRaspberryPi.variant == "5") "kernel";
  ...
  sdImage = {
    ...
    populateFirmwareCommands = ''
      ${config.boot.loader.nixosRaspberryPi.firmwarePopulateCmd} -c ${config.system.build.toplevel} -f ./firmware
    '';
    populateRootCommands = ''
      # create with a mount point for FIRMWARE
      mkdir -p ./files/boot/firmware
      ${config.boot.loader.nixosRaspberryPi.bootPopulateCmd} -c ${config.system.build.toplevel} -b ./files/boot
    '';
  };
}